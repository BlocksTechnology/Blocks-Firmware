...
inline void gcode_G51() {
  ...
  float xpos = parser.linearval('X', X_BED_SIZE/2);
  float ypos = parser.linearval('Y', FRONT_PROBE_BED_POSITION);

  zprobe_zoffset = probe_pt(xpos, ypos, raise_after, verbose_level);
...
static void load_filament_blocks(const float &load_length = 0, const float &initial_extrude_length = 0, const int8_t max_beep_count = 0) {
    ...
    if (load_length != 0) {
      ...
      /// Load filament
      destination[E_AXIS] += 110;
      buffer_line_to_destination(ADVANCED_PAUSE_PURGE_FEEDRATE);
      planner.synchronize();
    ...
    #if ENABLED(ULTIPANEL) && ADVANCED_PAUSE_PURGE_LENGTH > 0

      float extrude_length = initial_extrude_length;
    if (extrude_length > 0) {
          // "Wait for filament extrude"
          lcd_advanced_pause_show_message(ADVANCED_PAUSE_MESSAGE_PURGE);
...
inline void gcode_M710() {
  ...
  #ifdef FILAMENT_CHANGE_FAST_LOAD_LENGTH
    const float load_length = FILAMENT_CHANGE_FAST_LOAD_LENGTH + FILAMENT_CHANGE_SLOW_LOAD_LENGTH;
  #endif
  ;
  ...
  set_destination_from_current();
  buffer_line_to_destination(120);
  planner.synchronize();

  current_position[X_AXIS] = 910;
  current_position[Y_AXIS] = 40;
  set_destination_from_current();
  buffer_line_to_destination(120);
  planner.synchronize();
  ...
  load_filament_blocks(load_length, ADVANCED_PAUSE_PURGE_LENGTH, beep_count);
...
inline void gcode_M711() {
  ...
  ensure_safe_temperature(); // wait for extruder to heat up before unloading
    ...
    if (unload_length != 0) {
      ...
      set_destination_from_current();
      buffer_line_to_destination(FILAMENT_CHANGE_UNLOAD_FEEDRATE);
      planner.synchronize();
