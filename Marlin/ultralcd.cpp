...
uint16_t max_display_update_time = 0;
  ...
  int filament_control_lu;
  bool filament_loaded;
  bool filament_load_pla;
  bool filament_load_etc;
  bool check_load;
...
void lcd_quick_feedback(const bool clear_buttons) {
  ...
  inline void line_to_z(const float &z) {
    ...
    void lcd_sdcard_stop() {
      ...
      enqueue_and_echo_commands_P(PSTR("M42 P6 S0"));
  ...
  void lcd_main_menu() {
    ...
    MENU_ITEM(submenu, "Plate", lcd_level_plate);
   ...
   */
    ...
    void lcd_assisted_next_point_message() {
      START_SCREEN();
      STATIC_ITEM("Next Point");
      END_SCREEN();
    }

    void lcd_assisted_started_message() {
      START_SCREEN();
      STATIC_ITEM("");
    ...
    void lcd_assisted_probing_message() {
      START_SCREEN();
      STATIC_ITEM("");
      STATIC_ITEM("");
      STATIC_ITEM("Probing");
    ...
    void lcd_assisted_piece_center_message() {
      START_SCREEN();
    ...
    void lcd_assisted_leveling_done_message() {
      START_SCREEN();
      STATIC_ITEM("");
      STATIC_ITEM("Assisted Bed");
    ...
    void lcd_assisted_turn_degrees_message() {
      START_SCREEN();
      STATIC_ITEM(" ");
      lcd_print("Turn ");
    ...
    void lcd_assisted_clean_nozzle_message() {
      START_SCREEN();
    ...
    void lcd_assisted_manual_leveling_message() {
      START_SCREEN();
      STATIC_ITEM("");
    ...
    FORCE_INLINE screenFunc_t assisted_ap_message_screen(const AdvancedAssistedBedLevelingMessage message) {
      switch (message) {
        case OPTION_NEXT_POINT: return lcd_assisted_next_point_message;
        case STARTED: return lcd_assisted_started_message;
        case PROBING: return lcd_assisted_probing_message;
        case PIECE_CENTER: return lcd_assisted_piece_center_message;
        case TURN_DEGREES: return lcd_assisted_turn_degrees_message;
        case BED_LEVELING_DONE: return lcd_assisted_leveling_done_message;
        case CLEAN_NOZZLE_TWEEZERS: return lcd_assisted_clean_nozzle_message;
        case MANUAL_BED_LEVELING: return lcd_assisted_manual_leveling_message;
        case MESSAGE_STATUS:
        default: break;
      }
      return NULL;
    ...
    void lcd_assisted_bed_leveling(
      const AdvancedAssistedBedLevelingMessage message,
      const AdvancedPauseMode mode/*=ADVANCED_PAUSE_MODE_PAUSE_PRINT*/,
      const uint8_t extruder/*=active_extruder*/
    ) {
      advanced_pause_mode = mode;
      hotend_status_extruder = extruder;
      const screenFunc_t next_screen = assisted_ap_message_screen(message);
      if (next_screen) {
        defer_return_to_status = true;
        lcd_goto_screen(next_screen);
      }
...
void lcd_set_offset_screen() {
  ...
  if (lcdDrawUpdate) lcd_implementation_drawedit(PSTR("Z"), ftostr52((-zprobe_zoffset)+current_position[Z_AXIS]));
  if (lcd_clicked) {
    SERIAL_ECHO(zprobe_zoffset);
...
static void lcd_level_plate()
{
  START_MENU();
  MENU_ITEM(submenu, MSG_BACK, lcd_main_menu);
  MENU_ITEM(gcode, "Manual Level Plate", PSTR("G50"));
  MENU_ITEM(submenu, "Nozzle Adjustment", nozzle_adjustment_clean_nozzle);
  MENU_ITEM(gcode, "Reset BLTouch", PSTR("M280 S160 P0"));
